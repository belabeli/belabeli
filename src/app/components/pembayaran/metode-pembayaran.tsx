"use client";
import setUpQris from "@/api/pembayaran/metodePembayaran/getQris";
...
const PaymentDropdown: React.FC<PaymentDropdownProps> = ({
  ...
  const handlePaymentSelect = (method: string) => {
    // Toggle selection: jika yang di-click sama dengan yang sudah dipilih, maka unselect.
    if (selectedPayment === method) {
      setSelectedPayment(null);
      if (onPaymentSelect) onPaymentSelect(""); // Kirimkan string kosong jika tidak ada yang dipilih
    } else {
      setSelectedPayment(method);
      if (onPaymentSelect) onPaymentSelect(method);
    }
  };

  console.log("pembayarannya metode = ", selectedPayment);
  selectedPaymentMethod(selectedPayment);

  // async function handleQris() {
  //   const response = await setUpQris();

  //   console.log(response);
  // }

  // if (selectedPayment == "QRIS") {
  //   console.log("Dengan Qris");
  ...
  return (
    <div className="p-4 border-gray-300 relative">
      ...
      <div className="mb-2">
        ...
        >
          <span className="py-2 flex items-center gap-2">
            ...
            >
              <rect y="0.5" width="24" height="23" rx="4" fill="#51D7B1"></rect>
              <path
          ...
          <div className="mt-2 bg-white border rounded-md shadow-md">
            {["BCA", "Mandiri", "BNI", "BRI"].map((bank) => (
              <div
                ...
                onClick={() => handlePaymentSelect(`${bank} Virtual Account`)}
              >
          ...
          <div className="mt-2 bg-white border rounded-md shadow-md">
            {["BCA", "Mandiri", "BNI", "BRI"].map((bank) => (
              <div
                key={bank}
                className="flex justify-between items-center py-2 px-4 hover:bg-gray-100"
                onClick={() => handlePaymentSelect(`${bank} Bank Transfer`)}
              >
                <div className="flex items-center">
                  <img
                    src={`/${bank.toLowerCase()}.jpg`}
      ...
      {/* QRIS Option */}
      <div
        className="flex justify-between items-center py-2 px-4 bg-white border rounded-md"
