...
const CategoryBarProvince: React.FC<CategoryBarProps> = ({ title, array }) => {
  ...
  const handleButtonClick = (text: string) => {
    if (selectedId === text) {
        ...
        setSelectedId(text); 
        localStorage.setItem("province", `province=${text}`);
      }
  };

  useEffect(() => {
    const lastProvince = localStorage.getItem("province") || '';
    const clearedProvince = lastProvince.replace("province=", "");
    setSelectedId(clearedProvince); 
  }, []);
  ...
  return (
    <div className="overflow-auto">
      ...
      <div className="flex flex-wrap justify-evenly py-2 gap-2">
        {array.slice(0, 4).map((provinsi) => (
          <button
          key={provinsi.text}
          type="button"
          onClick={() => handleButtonClick(provinsi.text)}
          className={`h-fit w-fit py-2 px-8 ${
            selectedId === provinsi.text ? "bg-[#BFEDE0]" : "bg-[#E2F8F2]"
...
export default CategoryBarProvince;