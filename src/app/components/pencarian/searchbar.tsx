...
import CategoryHorizontal from './category-h-bar';
interface Product {
    id: number;
    name: string;
    slug: string;
...
const Searchbar = () => {
    ...
    useEffect(() => {
        const slug = localStorage.getItem('search') || '';
        setIsLoading(true);
        if(slug!== ''){
            setKeyword(slug);
    ...
    const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      timeoutRef.current = setTimeout(async () => {
        if (keyword.length >= 2) {
          setIsLoading(true); 
          try {
            const response = await getAutoComplete({ search: keyword });
            if (response.code === 200) {
    ...
    return (
        ...
        <div className={`flex flex-wrap ${disabled}`}>
            ...
            {isLoading ? (
                ...
                suggestions.map((product) => (
                    <div key={product.id} className="flex flex-wrap justify-between cursor-pointer">
                        <a
                            onClick={() => handleSearch(product.slug)} // Pass the slug to handleSearch
                            className="flex flex-wrap justify-left border-b border-b-gray-200 w-full py-2"
                        >
                ...
                <>
                    ...
                    <a
                        href={`/homepage/pencarian`}
                        className="bg-[#51D7B1] text-white py-3 px-5 text-sm rounded-lg"
                    >
                        Coba lagi
                    </a>
                    </div>
        ...
        <div className={`flex flex-wrap ${appear}`}>
