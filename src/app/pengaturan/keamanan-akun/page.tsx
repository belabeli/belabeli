...
import verifyGantiSandi from '@/api/settings/postVerifyGantiSandi';
...
const KeamananAkun = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentPassword && newPassword && confirmPassword) {
      if (newPassword === confirmPassword) {
        ...
        console.log('Kata sandi baru dan konfirmasi kata sandi tidak cocok.');
      }
    } else {
  ...
  const HandleRequestOTPEmail = async () => {
    setIsLoading(true);
    const response = await verifyGantiSandi({ otps_id: otpId, new_password: newPassword, new_password_confirmation: confirmPassword});
    console.log("response verify kata sandi error", response.error);
    console.log("response verify kata sandi success", response.data);
    const logout = await LogoutAfterCP()
    if(!logout.error){
      const deleteCookies = await deleteToken();
    ...
    <LayoutUtama>
      ...
      <div className="flex flex-col gap-2 items-center">
        <div className="w-[190px] h-[18px] relative mb-4">
          ...
          <div className="w-[185px] h-3.5 left-[2px] top-[2px] absolute bg-[#51D7B1] rounded-xl" />
        </div>
          <h1 className="mt-10 text-xl text-center font-nunitoBold">Kata Sandi Baru</h1>
          <span className="w-full text-center text-md mt-2 px-8 text-gray-500">
            <p className="font-nunito">Buat kata sandi baru Anda untuk mengamankan akun</p>
          </span>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="relative mt-12">
            ...
            <label
              htmlFor="newPassword"
              className="absolute left-4 top-3 transform -translate-y-1/2 text-sm text-gray-500 transition-all duration-200
          ...
          <div className="text-sm mt-4">
            ...
            <p className={passwordHasLetter && passwordHasNumber ? "text-green-500" : "text-red-500"}>
              • Min, 1 huruf dan 1 angka
            </p>
            <p className={passwordHasSpecialChar ? "text-green-500" : "text-red-500"}>
              • 1 karakter khusus
            </p>

            <p className={isMatched ? "text-green-500" : "text-red-500"}>
              • {isMatched ? "Password sesuai" : "Password tidak sesuai"}
            </p>
          </div>
      ...
      className="w-full bg-[#51D7B1] hover:bg-emerald-600 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center"
...
{isPopupVisible && (
  <div
    className="fixed inset-0 z-50 flex justify-center items-end min-h-screen bg-black bg-opacity-50"
    onClick={closePopup}
  >
    <div
      className="bg-white w-full max-w-md h-auto p-6 rounded-t-[24px] shadow-lg transform transition-transform duration-300 ease-out"
